...
$david = $voice.GetVoices() | Where-Object {
    $_.GetDescription() -like "*David*"
} | Select-Object -First 1

if (-not $david) {
    throw "Microsoft David not found in SAPI"
}
$voice.Voice = $david
$voice.Speak("", 1) | Out-Null
$voice.Speak('${escaped}')
`;
...
// Converts parseMessageRuns() output back into plain text.
...
ipcMain.handle('tts-confirmation', (_, text) => {
    if (win)
        win.webContents.send("tts-confirmation", { text });
});
...
ipcMain.handle("dequeue-chat-message", () => {
    return uiMessageQueue.shift() ?? null;
});
  ...
  try {
   ...
   for (const action of actions) {
	...
	if (renderer.authorBadges)
		...
		const bannedpeople = ['@vista_guyn', '@vistguya21']

		const plainText = messageRunsToText(message);

		if (!bannedpeople.includes(authorhandler))
			if (plainText.startsWith("!tts ")) {
				const tts = plainText.substring(5).trim();

				if (tts.length) {
					speak(tts);

					if (win) {
						win.webContents.send("tts-confirmation", {
							text: tts
						});
					}
				}
